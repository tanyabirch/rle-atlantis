...
- Access to the GCP project (e.g., `rle-atlantis`)
...
```bash
gcloud projects add-iam-policy-binding "rle-atlantis" \
  --member="serviceAccount:github-actions-rle@rle-atlantis.iam.gserviceaccount.com" \
...
```bash
gcloud projects describe rle-atlantis --format="value(projectNumber)"
```
...
gcloud iam service-accounts add-iam-policy-binding \
  "github-actions-rle@rle-atlantis.iam.gserviceaccount.com" \
  --project="rle-atlantis" \
...
| `GCP_SERVICE_ACCOUNT` | `github-actions-rle@rle-atlantis.iam.gserviceaccount.com` |
...
```
Caller does not have required permission to use project rle-atlantis.
